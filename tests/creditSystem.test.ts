...
describe('CreditSystem — Credit 資源 / 耗盡 / 投幣', () => {
  ...
  it('壞版對照：耗盡時 canAttack 必須 false（不可只看 credit>0 忽略耗盡）', () => {
    ...
    const badCanAttack = sys.getCredit(0) >= 0; // 壞版邏輯
    expect(badCanAttack).toBe(true);
    expect(sys.canAttack(0)).not.toBe(badCanAttack); // 兩者不同 → 證明耗盡判斷有作用
  });
});
...
function makeControllable() {
  ...
  const ctx = {
    ...
    player: { playerId: 0, setOutOfCreditTint: (v: boolean) => tints.push(v) },
  } as unknown as GameContext;
  const sys = new CreditSystem();
  sys.init(ctx);
...
function consumeTo(sys: CreditSystem, target: number): void {
  ...
  for (let i = 0; i < times; i += 1) sys.consumeOnHit(0);
}
...
describe('CreditSystem — 扣 Credit 邊界（1→0 進耗盡 / 2→1 不耗盡 / clamp）', () => {
  it('credit=2 → 命中 → 1，【不】進耗盡、仍可攻擊/行動（邊界另一側）', () => {
    ...
    consumeTo(sys, 2);
    expect(sys.getCredit(0)).toBe(2);
    expect(sys.isOutOfCredit(0)).toBe(false);
    sys.consumeOnHit(0); // 2 → 1
    expect(sys.getCredit(0)).toBe(1);
    expect(sys.isOutOfCredit(0)).toBe(false); // 1 > 0 → 不耗盡
    expect(sys.canAttack(0)).toBe(true);
    expect(sys.canAct(0)).toBe(true);
  });
  ...
  it('耗盡後再 consumeOnHit：守 0 不變負（clamp + guard，扣多次仍 0）', () => {
    const { sys } = makeControllable();
    consumeTo(sys, 1);
    sys.consumeOnHit(0); // → 0 進耗盡
    sys.consumeOnHit(0);
    sys.consumeOnHit(0);
    expect(sys.getCredit(0)).toBe(0); // 不會變負
    expect(sys.isOutOfCredit(0)).toBe(true);
  });
});
...
describe('CreditSystem — CanAttack / canAct 閘門', () => {
  ...
  it('耗盡：canAttack=false 且 canAct=false（移動也鎖）', () => {
    ...
    expect(sys.canAttack(0)).toBe(false);
    expect(sys.canAct(0)).toBe(false); // 耗盡連移動都鎖（這條就是閘門鑑別點）
  });
});
...
describe('CreditSystem — 投幣 AddCredit 與耗盡解除', () => {
  it('一般狀態投幣：credit += 100（非耗盡不誤觸解除流程）', () => {
    ...
    consumeTo(sys, 10);
    sys.addCredit(0, COIN_INSERT_AMOUNT);
    expect(sys.getCredit(0)).toBe(10 + COIN_INSERT_AMOUNT);
    expect(sys.isOutOfCredit(0)).toBe(false);
  });
  ...
  it('耗盡狀態投幣 → credit>0 → 自動解除耗盡（投幣後恰好 >0）', () => {
    const { sys } = makeControllable();
    consumeTo(sys, 1);
    sys.consumeOnHit(0); // 進耗盡 credit=0
    expect(sys.isOutOfCredit(0)).toBe(true);
    sys.addCredit(0, COIN_INSERT_AMOUNT);
    expect(sys.getCredit(0)).toBe(COIN_INSERT_AMOUNT); // > 0
    expect(sys.isOutOfCredit(0)).toBe(false); // 解除
    expect(sys.canAttack(0)).toBe(true);
    expect(sys.canAct(0)).toBe(true);
  });
...
describe('CreditSystem — 耗盡倒數', () => {
  it('倒數中（未到 0）仍耗盡、getCountdown 遞減', () => {
    const { sys } = makeControllable();
    consumeTo(sys, 1);
    sys.consumeOnHit(0); // 進耗盡，countdown=10
    sys.update(3); // 走 3 秒
    expect(sys.isOutOfCredit(0)).toBe(true); // 尚未到 0
...
describe('CreditSystem — 帳本不變量：倒數期間互動', () => {
  ...
  function enterCountdown(elapsed = 4) {
    ...
    consumeTo(ctl.sys, 1);
    ctl.sys.consumeOnHit(0); // → 0 進耗盡，countdown=OUT_OF_CREDIT_COUNTDOWN
  ...
  it('不變量：倒數中命中(consumeOnHit)不誤扣、不改狀態、不影響倒數', () => {
    ...
    const creditBefore = sys.getCredit(0); // 0
    const countdownBefore = sys.getCountdown(0);
    expect(sys.isOutOfCredit(0)).toBe(true);
    sys.consumeOnHit(0);
    sys.consumeOnHit(0);
    expect(sys.getCredit(0)).toBe(creditBefore); // 仍 0，不變負/不誤扣
    expect(sys.isOutOfCredit(0)).toBe(true);
    expect(sys.getCountdown(0)).toBeCloseTo(countdownBefore);
  });
  ...
  it('不變量：倒數中投幣 → 中止倒數 + 解鎖（credit>0、countdown 歸 0、閘門開）', () => {
    ...
    expect(sys.getCountdown(0)).toBeGreaterThan(0);
    coinRef.pressed = true;
    sys.update(0.016);
