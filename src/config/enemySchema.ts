...
// ---- 遊戲讀取端：匯入 override 優先（enemies JSON 化，接通怪物編輯→遊戲生效）--------------
...
export function resolveEnemies(
  override: unknown,
  packaged: Record<string, EnemyAIConfig> = ENEMY_AI,
...
let resolvedEnemiesCache: Record<string, EnemyAIConfig> | null = null;

/**
 * 遊戲端取得敵人設定表（override 優先 + cache）。Enemy 建構子用。
 * 首次呼叫讀 localStorage override（loadOverride）→ resolveEnemies → 快取；之後回快取。
 */
export function getResolvedEnemies(): Record<string, EnemyAIConfig> {
  if (resolvedEnemiesCache) return resolvedEnemiesCache;
  const override = loadOverride(EDITOR_STORE_KEYS.enemies);
  if (override === null) {
    // 無 override：警告靜默，直接用打包預設（不快取 null 以外的特殊處理不需要）。
  } else {
    const result = validateEnemies(override);
    if (!result.ok) console.warn('[enemySchema] enemies override 驗證失敗，改用打包預設：', result.errors);
  }
  resolvedEnemiesCache = resolveEnemies(override);
  return resolvedEnemiesCache;
}

/** 取單一敵人設定（override 優先 + cache）；查無回 undefined（呼叫端 fallback）。 */
export function getResolvedEnemy(charKey: string): EnemyAIConfig | undefined {
